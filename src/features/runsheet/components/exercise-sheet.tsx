...
/** Label and increment for the count a step is done for; none for max efforts and video segments. */
export const countSetting = (s: ExerciseStep): { label: string; step: number } | null => {
  switch (s.forMode) {
    case 'reps':
    case 'amrap':
      return { label: s.perSide ? 'Reps (each side)' : 'Reps', step: 1 };
    case 'seconds':
      return { label: 'Seconds', step: 5 };
    case 'minutes':
      return { label: 'Minutes', step: 1 };
    case 'meters':
      return { label: 'Metres', step: 50 };
    case 'calories':
      return { label: 'Calories', step: 1 };
    default:
      return null;
  }
};
 ...
 */
export const ExerciseSheet = ({ step: s, onOpenChange, onTarget, onIncline, onForValue, target, incline, note }: ExerciseSheetProps) => (
  <Sheet open={!!s} onOpenChange={onOpenChange} title={s?.exercise.name}>
    {s && (
      <div className="space-y-3">
        ...
        {s && onForValue && countSetting(s) && (
          <div className="flex items-center justify-between gap-3 rounded-card border border-line bg-surface px-3 py-2.5">
            <span className="text-[15px] font-semibold">{countSetting(s)!.label}</span>
            <Stepper aria-label={countSetting(s)!.label} value={s.forValue} step={countSetting(s)!.step} min={1} max={9999} onChange={onForValue} />
          </div>
        )}
